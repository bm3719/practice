...
drop procedure dc_CheckStoredProcs;
DELIMITER //
...
drop procedure dc_CheckAccounts;
DELIMITER //
...
drop procedure dc_CheckItemsAllChars
DELIMITER //
...
drop procedure dc_CheckItemsChar
DELIMITER //
create procedure dc_CheckItemsChar ( IN CharName VARCHAR(64) CHARACTER SET sjis )
BEGIN
  select c.char_name, ci.item_name, ci.item_id, ci.enchantlvl, ci.count from characters c, 
...
drop procedure dc_CheckDropMops 
DELIMITER //
...
drop procedure dc_CheckMopDrops 
DELIMITER //
...
drop procedure dc_SetUnstuckChar
DELIMITER //
create procedure dc_SetUnstuckChar ( IN CharName VARCHAR(64) CHARACTER SET sjis )
BEGIN
...
-- rename a character
drop procedure dc_SetCharRename 
DELIMITER //
create procedure dc_SetCharRename ( 
	IN OldCharName VARCHAR(64) CHARACTER SET sjis,
        IN NewCharName VARCHAR(64) CHARACTER SET sjis )
BEGIN
